...
from optimizer.sio_items import assemble_sio_item_stats, item_state_from_profile
...
def _payload(profile: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], list[str], dict[str, Any], dict[str, Any]]:
    sio = profile.get("sio_ce") if isinstance(profile.get("sio_ce"), Mapping) else {}
    ...
    item_result: dict[str, Any] = {"detail": {}, "warnings": [], "stats_stage": stage}
    if item_state_from_profile(profile) and stage not in {"post_24804", "sio_post_24804", "post_24804_items"}:
        item_result = assemble_sio_item_stats(profile, stats)
        stats = dict(item_result["stats"])
        warnings.extend(item_result["warnings"])
        stage = str(item_result["stats_stage"])
    ...
    for row in (profile.get("attack"), sio.get("attack")):
        if isinstance(row, Mapping):
            attack.update(row)
    warnings.extend(mount_result["warnings"])
    ...
    if stage not in {"post_24804", "sio_post_24804", "post_24804_items"}:
    ...
    return stats, attack, dict(direct) if isinstance(direct, Mapping) else {}, warnings, mount_result, item_result
...
def calculate_clan_expedition_damage(profile: Mapping[str, Any]) -> dict[str, Any]:
    ...
    stats, attack, direct_map, warnings, mount_result, item_result = _payload(profile)
    ...
    sio_payload = profile.get("sio_ce")
    passive = _num(sio_payload.get("passive_multiplier", 1.0), 1.0) if isinstance(sio_payload, Mapping) else 1.0
    if passive <= 0:
        passive = 1.0
    if not isinstance(sio_payload, Mapping) or "passive_multiplier" not in sio_payload:
    ...
    return {
        ...
        "item_detail": item_result.get("detail", {}),
